      ...
      <div class="pipeline-container">
        ...
        <div v-else-if="pipelines !== null && pipelines.length > 0">
          <div v-for="pipeline in pipelines">
            <pipeline-view-stable :pipeline="pipeline" :project="project" />
          </div>
        </div>
...
<script>
  ...
  import PipelineViewStable        from './pipeline-view-stable';
  ...
  export default {
    ...
    methods: {
      ...
      async fetchPipelines() {
        ...
        const stableVersion = getQueryParameter('stableVersion')!==null ? getQueryParameter('stableVersion') :'';
        if (pipelines.length === 0) {
          this.$data.pipelines = [];
        } else {
            const filteredPipelines = [];
          for (const pipeline of pipelines) {             
                if (pipeline.ref == stableVersion && pipeline.status=='success') {                  
